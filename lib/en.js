$(function(){
    ...
    En.prototype.draw = function(){
        ...
        if(this.pie_array != null) this.fill_pie();
        if(this.has_image) this.draw_image();
        if(this.text_array != null) this.fill_text();
    };
    En.prototype.to_pie = function(pies, start){
        this.pie_array = [];
        this.pie_start = start ? parseInt(start) / 360 : 0;
        if(typeof pies == 'number'){
            for(var i=0;i<parseInt(pies);i++){
                this.pie_array.push({ size : 1 / parseInt(pies), color : this.color });
            }
        } else if(typeof pies == 'array'){
            for(var i=0;i<pies.length;i++){
                var pie_color = 'color' in this.pies[i] ? this.pies[i].color : this.color;
                this.pie_array.push({ size : pies[i] / 100, color : pie_color });
            }
        }
    };
    En.prototype.set_pie_color = function(index, color){
        if(index < 0 || index >= this.pie_array.length) return;
        this.pie_array[index].color = color;
        en_manager.refresh(this.target);
    };
    En.prototype.fill_pie = function(){
        if(this.pie_array == null || this.pie_array.length < 1) return;
        var start = 360;
        for(var i=0;i<this.pie_array.length;i++){
            var pie   = this.pie_array[i];
            var begin = Math.PI * (- 0.5 + this.pie_start + 2 * start);
            var close = Math.PI * (- 0.5 + this.pie_start + 2 * (start + pie.size));
            this.context.beginPath();
            this.context.moveTo(this.x, this.y);
            this.context.arc(this.x, this.y, this.radius + this.border, begin, close, false);
            this.context.lineTo(this.x, this.y);
            this.context.closePath();
            this.context.fillStyle = pie.color;
            this.context.fill();
            start -= pie.size;
        }
    };
    En.prototype.set_text = function(options){
        var params = typeof options == 'array' ? options : [options];
        var option = { value : '', x : 0, y : 0, font : '10px Arial', color : '#000' };
        if(this.text_array == null) this.text_array = [];
        for(var i=0;i<params.length;i++){
            this.text_array.push($.extend(option, params[i]));
        }
        en_manager.refresh(this.target);
    };
    En.prototype.remove_text = function(index){
        if(index == undefined){
            this.text_array = null;
        } else {
            this.text_array = this.text_array.splice(index, 1);
        }
        en_manager.refresh(this.target);
    };
    En.prototype.fill_text = function(){
        if(null == this.text_array) return;
        var that = this;
        $.each(this.text_array, function(index, text){
          var dx = that.x + text.x;
          var dy = that.y - text.y;
          that.context.font = text.font;
          that.context.fillStyle = text.color;
          that.context.fillText(text.value, dx, dy);
        });
    };
    ...
    EnManager.prototype.apply_events = function(target){
        ...
        var convert_table = { mousedown : 'touchstart', mousemove : 'touchmove', mouseup : 'touchend' };
        var convert = function(key){
            return this_is_smartphone || false == key in convert_table ? key : convert_table[key];
        };
        $.each(ev_keys, function(index, en_event){
            $(target).bind(en_event, function(e){
                ...
                $.each(en_list, function(index, en){
                    ...
                    en.fire(convert(en_event), event_obj);
                });
            });
        });
    };
    ...
    $.fn.en = function(options){
        ...
        if(index < 0) return;
        var en_options = apply_options({ x : 50, y : 50, color : '#333', radius : 50, type : 'fill', layer : 1 }, options); 
